"""
Prometheus instrumentation for pyfaulttolerance components.

This module provides listeners that emit Prometheus metrics for all pyfaulttolerance
fault tolerance components. Install with: pip install pyfaulttolerance[prometheus]
...
try:
    ...
    raise ImportError(
        "prometheus-client is required for Prometheus instrumentation. Install it with: pip install pyfaulttolerance[prometheus]"
    ) from e
...
if TYPE_CHECKING:
    ...
    from pyfaulttolerance.retry.counters import Counter as RetryCounter
...
class CircuitBreakerListener(BaseBreakerListener):
    ...
    def __init__(self, registry: CollectorRegistry | None = None) -> None:
        ...
        self._state_counter = Counter(
            name="pyfaulttolerance_circuitbreaker_state_transitions_total",
            documentation="Number of circuit breaker state transitions",
            labelnames=["component", "from_state", "to_state"],
            registry=registry,
        )
        self._success_counter = Counter(
            name="pyfaulttolerance_circuitbreaker_success_total",
...
class TimeoutListener(BaseTimeoutListener):
    ...
    def __init__(self, registry: CollectorRegistry | None = None) -> None:
        ...
        self._timeout_counter = Counter(
            name="pyfaulttolerance_timeout_exceeded_total",
...
class BulkheadListener(BaseBulkheadListener):
    ...
    def __init__(self, registry: CollectorRegistry | None = None) -> None:
        ...
        self._rejected_counter = Counter(
            name="pyfaulttolerance_bulkhead_rejected_total",
...
class FallbackListener(BaseFallbackListener):
    ...
    def __init__(self, registry: CollectorRegistry | None = None) -> None:
        ...
        self._fallback_counter = Counter(
            name="pyfaulttolerance_fallback_triggered_total",
...
def register_listeners(registry: CollectorRegistry | None = None) -> None:
    """
    Register Prometheus listeners for all pyfaulttolerance components.
    ...
    Example:
        ...
        # Now all pyfaulttolerance components will emit Prometheus metrics
    """
